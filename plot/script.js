...
async function init() {
	await fetch("./comics.json")
		...
		loadCard();
	let navHTML = "<a href='../index.html?lan=" + lan + "'> Dragalia Comic Index</a> | <form action='comic.html'><input type='number' name='no' placeholder='Jump to Comic...' pattern='[0-9]+'><input type='hidden' name='lan' value='" + lan + "'></form>"
...
function loadCard(numCards = 24){
	...
	for (i = 0; i < numCards; i++){
		...
		if (currentComic < comicArray[comicArray.length]) {return;}
